...
expName = 'REPSWITCH1_version2_list2'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\2023\\REPSWITCH12092023\\REPSWITCH_excgange\\REPSWITCH_excgange\\REPSWITCH1_version2_list2_lastrun.py',
...
# Make folder to store recordings from micRespv2l2
micRespv2l2RecFolder = filename + '_micRespv2l2_recorded'
if not os.path.isdir(micRespv2l2RecFolder):
    os.mkdir(micRespv2l2RecFolder)
...
textboxInstrMain1_2 = visual.TextBox2(
     ...
     pos=(0.42, 0.2358),     letterHeight=0.03,
     size=(None, None), borderWidth=2.0,
     color='white', colorSpace='rgb',
     opacity=None,
...
textboxInstrMain2_5 = visual.TextBox2(
     ...
     pos=(0.46, 0.22),     letterHeight=0.03,
     size=(None, None), borderWidth=2.0,
     color='white', colorSpace='rgb',
     opacity=None,
     bold=True, italic=False,
...
textboxInstrMain2_6 = visual.TextBox2(
     ...
     pos=(0.74, 0.2),     letterHeight=0.03,
     size=(None, None), borderWidth=2.0,
     color='white', colorSpace='rgb',
     opacity=None,
     bold=False, italic=False,
...
textboxInstrMain2_10 = visual.TextBox2(
     ...
     pos=(0.52, 0.1),     letterHeight=0.03,
     size=(None, None), borderWidth=2.0,
     color='white', colorSpace='rgb',
     opacity=None,
     bold=True, italic=False,
...
textboxInstrMain2_11 = visual.TextBox2(
     ...
     pos=(0.63, 0.1),     letterHeight=0.03,
     size=(None, None), borderWidth=2.0,
     color='white', colorSpace='rgb',
     opacity=None,
     bold=False, italic=False,
...
micRespv2l2 = sound.microphone.Microphone(
...
data_file = "repswitch_V2L2.xlsx" # the rest accordingly
...
trialsREPSWITCH = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('repswitch_V2L2.xlsx'),
...
for thisTrialsREPSWITCH in trialsREPSWITCH:
    ...
    # keep track of which components have finished
    trialComponents = [polygonColour, polygonWhite, imageObject, polygonType, textInput, keyResp, micRespv2l2]
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        # micRespv2l2 updates
        if micRespv2l2.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            micRespv2l2.frameNStart = frameN  # exact frame index
            micRespv2l2.tStart = t  # local t and not account for scr refresh
            micRespv2l2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(micRespv2l2, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.addData('micRespv2l2.started', t)
            # start recording with micRespv2l2
            micRespv2l2.start()
            micRespv2l2.status = STARTED
        if micRespv2l2.status == STARTED:
            # update recorded clip for micRespv2l2
            micRespv2l2.poll()
        if micRespv2l2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > micRespv2l2.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
                micRespv2l2.tStop = t  # not accounting for scr refresh
                micRespv2l2.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('micRespv2l2.stopped', t)
                # stop recording with micRespv2l2
                micRespv2l2.stop()
                micRespv2l2.status = FINISHED
        # Run 'Each Frame' code from codeMicRespv2l2
        if t > 3:
            continueRoutine = False
    ...
    # tell mic to keep hold of current recording in micRespv2l2.clips and transcript (if applicable) in micRespv2l2.scripts
    # this will also update micRespv2l2.lastClip and micRespv2l2.lastScript
    micRespv2l2.stop()
    tag = data.utils.getDateStr()
    micRespv2l2Clip = micRespv2l2.bank(
        tag=tag, transcribe='None',
        config=None
    )
    trialsREPSWITCH.addData('micRespv2l2.clip', os.path.join(micRespv2l2RecFolder, 'recording_micRespv2l2_%s.wav' % tag))
    # Run 'End Routine' code from codeSaveResp
    clipFilename = f"recording_V2L2_p{participant_number}_tr{str(count)}_{respModal}_{correctAns}_%s.wav" % tag
    #"recording_" + str(count) + ".wav"
    micRespv2l2.lastClip.save(os.path.join(micRespv2l2RecFolder, clipFilename))
    ...
    if (trialsREPSWITCH.thisN + 1) % 51 != 0 or trialsREPSWITCH.thisN == trialsREPSWITCH.nTotal - 1:
        ...
        # trialsREPSWITCH.thisN == trialsREPSWITCH.nTotal - 1 checks if the current trial is the last trial of the 
...
# save micRespv2l2 recordings
for tag in micRespv2l2.clips:
    for i, clip in enumerate(micRespv2l2.clips[tag]):
        clipFilename = 'recording_micRespv2l2_%s.wav' % tag
        # if there's more than 1 clip with this tag, append a counter for all beyond the first
        if i > 0:
            clipFilename += '_%s' % i
        clip.save(os.path.join(micRespv2l2RecFolder, clipFilename))
