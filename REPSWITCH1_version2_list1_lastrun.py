...
expName = 'REPSWITCH1_version2_list1'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\2023\\REPSWITCH12092023\\REPSWITCH_excgange\\REPSWITCH_excgange\\REPSWITCH1_version2_list1_lastrun.py',
...
# Make folder to store recordings from micRespv2l1
micRespv2l1RecFolder = filename + '_micRespv2l1_recorded'
if not os.path.isdir(micRespv2l1RecFolder):
    os.mkdir(micRespv2l1RecFolder)
...
textboxInstrMain2_11 = visual.TextBox2(
     ...
     pos=(0.64, 0.1),     letterHeight=0.03,
     size=(None, None), borderWidth=2.0,
     color='white', colorSpace='rgb',
     opacity=None,
     bold=False, italic=False,
...
micRespv2l1 = sound.microphone.Microphone(
...
data_file = "repswitch_V2L1.xlsx" # the rest accordingly
...
trialsREPSWITCH = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('repswitch_V2L1.xlsx'),
...
for thisTrialsREPSWITCH in trialsREPSWITCH:
    ...
    # keep track of which components have finished
    trialComponents = [polygonColour, polygonWhite, imageObject, polygonType, textInput, keyResp, micRespv2l1]
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        # micRespv2l1 updates
        if micRespv2l1.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            micRespv2l1.frameNStart = frameN  # exact frame index
            micRespv2l1.tStart = t  # local t and not account for scr refresh
            micRespv2l1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(micRespv2l1, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.addData('micRespv2l1.started', t)
            # start recording with micRespv2l1
            micRespv2l1.start()
            micRespv2l1.status = STARTED
        if micRespv2l1.status == STARTED:
            # update recorded clip for micRespv2l1
            micRespv2l1.poll()
        if micRespv2l1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > micRespv2l1.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
                micRespv2l1.tStop = t  # not accounting for scr refresh
                micRespv2l1.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('micRespv2l1.stopped', t)
                # stop recording with micRespv2l1
                micRespv2l1.stop()
                micRespv2l1.status = FINISHED
        # Run 'Each Frame' code from codeMicRespv2l1
        if t > 3:
            continueRoutine = False
    ...
    # tell mic to keep hold of current recording in micRespv2l1.clips and transcript (if applicable) in micRespv2l1.scripts
    # this will also update micRespv2l1.lastClip and micRespv2l1.lastScript
    micRespv2l1.stop()
    tag = data.utils.getDateStr()
    micRespv2l1Clip = micRespv2l1.bank(
        tag=tag, transcribe='None',
        config=None
    )
    trialsREPSWITCH.addData('micRespv2l1.clip', os.path.join(micRespv2l1RecFolder, 'recording_micRespv2l1_%s.wav' % tag))
    # Run 'End Routine' code from codeSaveResp
    clipFilename = f"recording_V2L1p{participant_number}_tr{str(count)}_{respModal}_{correctAns}_%s.wav" % tag
    #"recording_" + str(count) + ".wav"
    micRespv2l1.lastClip.save(os.path.join(micRespv2l1RecFolder, clipFilename))
...
# save micRespv2l1 recordings
for tag in micRespv2l1.clips:
    for i, clip in enumerate(micRespv2l1.clips[tag]):
        clipFilename = 'recording_micRespv2l1_%s.wav' % tag
        # if there's more than 1 clip with this tag, append a counter for all beyond the first
        if i > 0:
            clipFilename += '_%s' % i
        clip.save(os.path.join(micRespv2l1RecFolder, clipFilename))
