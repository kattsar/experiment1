...
expName = 'REPSWITCH1_version1_list1'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\2023\\REPSWITCH12092023\\REPSWITCH_excgange\\REPSWITCH_excgange\\REPSWITCH1_version1_list1_lastrun.py',
...
# Make folder to store recordings from micRespv1l1
micRespv1l1RecFolder = filename + '_micRespv1l1_recorded'
if not os.path.isdir(micRespv1l1RecFolder):
    os.mkdir(micRespv1l1RecFolder)
...
textboxInstrMain2_11 = visual.TextBox2(
     ...
     pos=(0.82, 0.1),     letterHeight=0.03,
     size=(None, None), borderWidth=2.0,
     color='white', colorSpace='rgb',
     opacity=None,
     bold=False, italic=False,
...
micRespv1l1 = sound.microphone.Microphone(
...
for thisTrialsREPSWITCH in trialsREPSWITCH:
    ...
    # keep track of which components have finished
    trialComponents = [polygonColour, polygonWhite, imageObject, polygonType, textInput, keyResp, micRespv1l1]
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        if micRespv1l1.status == STARTED:
            # update recorded clip for micRespv1l1
            micRespv1l1.poll()
        if micRespv1l1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > micRespv1l1.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
        ...
        # Run 'Each Frame' code from codeMicRespv1l1
        if t > 3:
            continueRoutine = False
    ...
    # tell mic to keep hold of current recording in micRespv1l1.clips and transcript (if applicable) in micRespv1l1.scripts
    # this will also update micRespv1l1.lastClip and micRespv1l1.lastScript
    micRespv1l1.stop()
    tag = data.utils.getDateStr()
    micRespv1l1Clip = micRespv1l1.bank(
        tag=tag, transcribe='None',
        config=None
    )
    trialsREPSWITCH.addData('micRespv1l1.clip', os.path.join(micRespv1l1RecFolder, 'recording_micRespv1l1_%s.wav' % tag))
    # Run 'End Routine' code from codeSaveResp
    clipFilename = f"recording_V1L1_p{participant_number}_tr{str(count)}_{respModal}_{correctAns}_%s.wav" % tag
    #"recording_" + str(count) + ".wav"
    micRespv1l1.lastClip.save(os.path.join(micRespv1l1RecFolder, clipFilename))
...
# save micRespv1l1 recordings
for tag in micRespv1l1.clips:
    for i, clip in enumerate(micRespv1l1.clips[tag]):
        clipFilename = 'recording_micRespv1l1_%s.wav' % tag
        # if there's more than 1 clip with this tag, append a counter for all beyond the first
        if i > 0:
            clipFilename += '_%s' % i
        clip.save(os.path.join(micRespv1l1RecFolder, clipFilename))
