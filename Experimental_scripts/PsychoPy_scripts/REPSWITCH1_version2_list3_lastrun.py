...
expName = 'REPSWITCH1_version2_list3'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\2023\\REPSWITCH12092023\\REPSWITCH_excgange\\REPSWITCH_excgange\\REPSWITCH1_version2_list3_lastrun.py',
...
# Make folder to store recordings from micRespv2l3
micRespv2l3RecFolder = filename + '_micRespv2l3_recorded'
if not os.path.isdir(micRespv2l3RecFolder):
    os.mkdir(micRespv2l3RecFolder)
...
micRespv2l3 = sound.microphone.Microphone(
...
data_file = "repswitch_V2L3.xlsx" # the rest accordingly
...
trialsREPSWITCH = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('repswitch_V2L3.xlsx'),
...
for thisTrialsREPSWITCH in trialsREPSWITCH:
    ...
    # keep track of which components have finished
    trialComponents = [polygonColour, polygonWhite, imageObject, polygonType, textInput, keyResp, micRespv2l3]
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        # micRespv2l3 updates
        if micRespv2l3.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            micRespv2l3.frameNStart = frameN  # exact frame index
            micRespv2l3.tStart = t  # local t and not account for scr refresh
            micRespv2l3.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(micRespv2l3, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.addData('micRespv2l3.started', t)
            # start recording with micRespv2l3
            micRespv2l3.start()
            micRespv2l3.status = STARTED
        if micRespv2l3.status == STARTED:
            # update recorded clip for micRespv2l3
            micRespv2l3.poll()
        if micRespv2l3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > micRespv2l3.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
                micRespv2l3.tStop = t  # not accounting for scr refresh
                micRespv2l3.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('micRespv2l3.stopped', t)
                # stop recording with micRespv2l3
                micRespv2l3.stop()
                micRespv2l3.status = FINISHED
        # Run 'Each Frame' code from codeMicRespv2l3
        if t > 3:
            continueRoutine = False
    ...
    # tell mic to keep hold of current recording in micRespv2l3.clips and transcript (if applicable) in micRespv2l3.scripts
    # this will also update micRespv2l3.lastClip and micRespv2l3.lastScript
    micRespv2l3.stop()
    tag = data.utils.getDateStr()
    micRespv2l3Clip = micRespv2l3.bank(
        tag=tag, transcribe='None',
        config=None
    )
    trialsREPSWITCH.addData('micRespv2l3.clip', os.path.join(micRespv2l3RecFolder, 'recording_micRespv2l3_%s.wav' % tag))
    # Run 'End Routine' code from codeSaveResp
    clipFilename = f"recording_V2L3_p{participant_number}_tr{str(count)}_{respModal}_{correctAns}_%s.wav" % tag
    #"recording_" + str(count) + ".wav"
    micRespv2l3.lastClip.save(os.path.join(micRespv2l3RecFolder, clipFilename))
...
# save micRespv2l3 recordings
for tag in micRespv2l3.clips:
    for i, clip in enumerate(micRespv2l3.clips[tag]):
        clipFilename = 'recording_micRespv2l3_%s.wav' % tag
        # if there's more than 1 clip with this tag, append a counter for all beyond the first
        if i > 0:
            clipFilename += '_%s' % i
        clip.save(os.path.join(micRespv2l3RecFolder, clipFilename))
