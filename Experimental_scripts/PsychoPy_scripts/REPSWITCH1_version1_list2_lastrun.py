...
expName = 'REPSWITCH1_version1_list2'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\2023\\REPSWITCH12092023\\REPSWITCH_excgange\\REPSWITCH_excgange\\REPSWITCH1_version1_list2_lastrun.py',
...
# Make folder to store recordings from micRespv1l2
micRespv1l2RecFolder = filename + '_micRespv1l2_recorded'
if not os.path.isdir(micRespv1l2RecFolder):
    os.mkdir(micRespv1l2RecFolder)
...
micRespv1l2 = sound.microphone.Microphone(
...
data_file = "repswitch_V1L2.xlsx" # the rest accordingly
...
trialsREPSWITCH = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('repswitch_V1L2.xlsx'),
...
for thisTrialsREPSWITCH in trialsREPSWITCH:
    ...
    # keep track of which components have finished
    trialComponents = [polygonColour, polygonWhite, imageObject, polygonType, textInput, keyResp, micRespv1l2]
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        # micRespv1l2 updates
        if micRespv1l2.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            micRespv1l2.frameNStart = frameN  # exact frame index
            micRespv1l2.tStart = t  # local t and not account for scr refresh
            micRespv1l2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(micRespv1l2, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.addData('micRespv1l2.started', t)
            # start recording with micRespv1l2
            micRespv1l2.start()
            micRespv1l2.status = STARTED
        if micRespv1l2.status == STARTED:
            # update recorded clip for micRespv1l2
            micRespv1l2.poll()
        if micRespv1l2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > micRespv1l2.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
                micRespv1l2.tStop = t  # not accounting for scr refresh
                micRespv1l2.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('micRespv1l2.stopped', t)
                # stop recording with micRespv1l2
                micRespv1l2.stop()
                micRespv1l2.status = FINISHED
        # Run 'Each Frame' code from codeMicRespv1l2
        if t > 3:
            continueRoutine = False
    ...
    # tell mic to keep hold of current recording in micRespv1l2.clips and transcript (if applicable) in micRespv1l2.scripts
    # this will also update micRespv1l2.lastClip and micRespv1l2.lastScript
    micRespv1l2.stop()
    tag = data.utils.getDateStr()
    micRespv1l2Clip = micRespv1l2.bank(
        tag=tag, transcribe='None',
        config=None
    )
    trialsREPSWITCH.addData('micRespv1l2.clip', os.path.join(micRespv1l2RecFolder, 'recording_micRespv1l2_%s.wav' % tag))
    # Run 'End Routine' code from codeSaveResp
    clipFilename = f"recording_V1L2_p{participant_number}_tr{str(count)}_{respModal}_{correctAns}_%s.wav" % tag
    #"recording_" + str(count) + ".wav"
    micRespv1l2.lastClip.save(os.path.join(micRespv1l2RecFolder, clipFilename))
...
# save micRespv1l2 recordings
for tag in micRespv1l2.clips:
    for i, clip in enumerate(micRespv1l2.clips[tag]):
        clipFilename = 'recording_micRespv1l2_%s.wav' % tag
        # if there's more than 1 clip with this tag, append a counter for all beyond the first
        if i > 0:
            clipFilename += '_%s' % i
        clip.save(os.path.join(micRespv1l2RecFolder, clipFilename))
