...
expName = 'REPSWITCH1_Practice1_version1'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\2023\\REPSWITCH12092023\\REPSWITCH_excgange\\REPSWITCH_excgange\\REPSWITCH1_Practice1_version1_lastrun.py',
...
# Make folder to store recordings from micPractice1v1
micPractice1v1RecFolder = filename + '_micPractice1v1_recorded'
if not os.path.isdir(micPractice1v1RecFolder):
    os.mkdir(micPractice1v1RecFolder)
...
micPractice1v1 = sound.microphone.Microphone(
...
data_file = "repswitch_practice1_version1.xlsx" 
...
trialsPractice = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('repswitch_practice1_version1.xlsx'),
...
for thisTrialsPractice in trialsPractice:
    ...
    # keep track of which components have finished
    practiceTrialComponents = [polygonCol, polygonWh, textCue, imagePractice, polygonText, textPractice, keyPractice, micPractice1v1]
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        # micPractice1v1 updates
        if micPractice1v1.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            micPractice1v1.frameNStart = frameN  # exact frame index
            micPractice1v1.tStart = t  # local t and not account for scr refresh
            micPractice1v1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(micPractice1v1, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.addData('micPractice1v1.started', t)
            # start recording with micPractice1v1
            micPractice1v1.start()
            micPractice1v1.status = STARTED
        if micPractice1v1.status == STARTED:
            # update recorded clip for micPractice1v1
            micPractice1v1.poll()
        if micPractice1v1.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > micPractice1v1.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
                micPractice1v1.tStop = t  # not accounting for scr refresh
                micPractice1v1.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('micPractice1v1.stopped', t)
                # stop recording with micPractice1v1
                micPractice1v1.stop()
                micPractice1v1.status = FINISHED
    ...
    # tell mic to keep hold of current recording in micPractice1v1.clips and transcript (if applicable) in micPractice1v1.scripts
    # this will also update micPractice1v1.lastClip and micPractice1v1.lastScript
    micPractice1v1.stop()
    tag = data.utils.getDateStr()
    micPractice1v1Clip = micPractice1v1.bank(
        tag=tag, transcribe='None',
        config=None
    )
    trialsPractice.addData('micPractice1v1.clip', os.path.join(micPractice1v1RecFolder, 'recording_micPractice1v1_%s.wav' % tag))
    # Run 'End Routine' code from codeSavePractice
    clipFilename = f"recording_Practice1v1_p{participant_number}_tr{str(count)}_{respModal}_{correctAns}_%s.wav" % tag
    #"recording_" + str(count) + ".wav"
    micPractice1v1.lastClip.save(os.path.join(micPractice1v1RecFolder, clipFilename))
...
routineTimer.reset()
# save micPractice1v1 recordings
for tag in micPractice1v1.clips:
    for i, clip in enumerate(micPractice1v1.clips[tag]):
        clipFilename = 'recording_micPractice1v1_%s.wav' % tag
        # if there's more than 1 clip with this tag, append a counter for all beyond the first
        if i > 0:
            clipFilename += '_%s' % i
        clip.save(os.path.join(micPractice1v1RecFolder, clipFilename))
