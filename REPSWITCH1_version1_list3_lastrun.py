...
expName = 'REPSWITCH1_version1_list3'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\2023\\REPSWITCH12092023\\REPSWITCH_excgange\\REPSWITCH_excgange\\REPSWITCH1_version1_list3_lastrun.py',
...
# Make folder to store recordings from micRespv1l3
micRespv1l3RecFolder = filename + '_micRespv1l3_recorded'
if not os.path.isdir(micRespv1l3RecFolder):
    os.mkdir(micRespv1l3RecFolder)
...
micRespv1l3 = sound.microphone.Microphone(
...
data_file = "repswitch_V1L3.xlsx" # the rest accordingly
...
trialsREPSWITCH = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('repswitch_V1L3.xlsx'),
...
for thisTrialsREPSWITCH in trialsREPSWITCH:
    ...
    # keep track of which components have finished
    trialComponents = [polygonColour, polygonWhite, imageObject, polygonType, textInput, keyResp, micRespv1l3]
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        # micRespv1l3 updates
        if micRespv1l3.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            micRespv1l3.frameNStart = frameN  # exact frame index
            micRespv1l3.tStart = t  # local t and not account for scr refresh
            micRespv1l3.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(micRespv1l3, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.addData('micRespv1l3.started', t)
            # start recording with micRespv1l3
            micRespv1l3.start()
            micRespv1l3.status = STARTED
        if micRespv1l3.status == STARTED:
            # update recorded clip for micRespv1l3
            micRespv1l3.poll()
        if micRespv1l3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > micRespv1l3.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
                micRespv1l3.tStop = t  # not accounting for scr refresh
                micRespv1l3.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('micRespv1l3.stopped', t)
                # stop recording with micRespv1l3
                micRespv1l3.stop()
                micRespv1l3.status = FINISHED
        # Run 'Each Frame' code from codeMicRespv1l3
        if t > 3:
            continueRoutine = False
    ...
    # tell mic to keep hold of current recording in micRespv1l3.clips and transcript (if applicable) in micRespv1l3.scripts
    # this will also update micRespv1l3.lastClip and micRespv1l3.lastScript
    micRespv1l3.stop()
    tag = data.utils.getDateStr()
    micRespv1l3Clip = micRespv1l3.bank(
        tag=tag, transcribe='None',
        config=None
    )
    trialsREPSWITCH.addData('micRespv1l3.clip', os.path.join(micRespv1l3RecFolder, 'recording_micRespv1l3_%s.wav' % tag))
    # Run 'End Routine' code from codeSaveResp
    clipFilename = f"recording_V1L3_p{participant_number}_tr{str(count)}_{respModal}_{correctAns}_%s.wav" % tag
    #"recording_" + str(count) + ".wav"
    micRespv1l3.lastClip.save(os.path.join(micRespv1l3RecFolder, clipFilename))
...
# save micRespv1l3 recordings
for tag in micRespv1l3.clips:
    for i, clip in enumerate(micRespv1l3.clips[tag]):
        clipFilename = 'recording_micRespv1l3_%s.wav' % tag
        # if there's more than 1 clip with this tag, append a counter for all beyond the first
        if i > 0:
            clipFilename += '_%s' % i
        clip.save(os.path.join(micRespv1l3RecFolder, clipFilename))
