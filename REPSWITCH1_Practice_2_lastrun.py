...
# --- Initialize components for Routine "WelcomePractice_2" ---
textWelcomePractice_2 = visual.TextStim(win=win, name='textWelcomePractice_2',
    text='Welcome to the third part of the experiment!',
    font='Open Sans',
...
textEndPractice_2 = visual.TextStim(win=win, name='textEndPractice_2',
    text='Great! \n\nYou finished the third part of the experiment!\n\nYou can now move to the fourth and last part of this experiment!\n\nPress SPACE to end this part.',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
# --- Prepare to start Routine "WelcomePractice_2" ---
continueRoutine = True
routineForceEnded = False
# update component parameters for each repeat
# keep track of which components have finished
WelcomePractice_2Components = [textWelcomePractice_2]
for thisComponent in WelcomePractice_2Components:
...
while continueRoutine and routineTimer.getTime() < 2.0:
    ...
    # *textWelcomePractice_2* updates
    if textWelcomePractice_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        textWelcomePractice_2.frameNStart = frameN  # exact frame index
        textWelcomePractice_2.tStart = t  # local t and not account for scr refresh
        textWelcomePractice_2.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(textWelcomePractice_2, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'textWelcomePractice_2.started')
        textWelcomePractice_2.setAutoDraw(True)
    if textWelcomePractice_2.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > textWelcomePractice_2.tStartRefresh + 2-frameTolerance:
            # keep track of stop time/frame for later
            textWelcomePractice_2.tStop = t  # not accounting for scr refresh
            textWelcomePractice_2.frameNStop = frameN  # exact frame index
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'textWelcomePractice_2.stopped')
            textWelcomePractice_2.setAutoDraw(False)
...
# --- Ending Routine "WelcomePractice_2" ---
for thisComponent in WelcomePractice_2Components:
...
for thisTrialsPractice_2 in trialsPractice_2:
    ...
    while continueRoutine and routineTimer.getTime() < 3.0:
        ...
        if micPractice_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > micPractice_2.tStartRefresh + 2.0-frameTolerance:
                # keep track of stop time/frame for later
...
# update component parameters for each repeat
# keep track of which components have finished
EndPractice_2Components = [textEndPractice_2]
...
while continueRoutine and routineTimer.getTime() < 3.0:
    ...
    if textEndPractice_2.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > textEndPractice_2.tStartRefresh + 3-frameTolerance:
            # keep track of stop time/frame for later
            textEndPractice_2.tStop = t  # not accounting for scr refresh
            textEndPractice_2.frameNStop = frameN  # exact frame index
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'textEndPractice_2.stopped')
            textEndPractice_2.setAutoDraw(False)
