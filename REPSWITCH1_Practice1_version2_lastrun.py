...
expName = 'REPSWITCH1_Practice1_version2'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\2023\\REPSWITCH12092023\\REPSWITCH_excgange\\REPSWITCH_excgange\\REPSWITCH1_Practice1_version2_lastrun.py',
...
# Make folder to store recordings from micPractice1v2
micPractice1v2RecFolder = filename + '_micPractice1v2_recorded'
if not os.path.isdir(micPractice1v2RecFolder):
    os.mkdir(micPractice1v2RecFolder)
...
micPractice1v2 = sound.microphone.Microphone(
...
data_file = "repswitch_practice1_version2.xlsx" 
...
trialsPractice = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions('repswitch_practice1_version2.xlsx'),
...
for thisTrialsPractice in trialsPractice:
    ...
    # keep track of which components have finished
    practiceTrialComponents = [polygonCol, polygonWh, textCue, imagePractice, polygonText, textPractice, keyPractice, micPractice1v2]
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        # micPractice1v2 updates
        if micPractice1v2.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            micPractice1v2.frameNStart = frameN  # exact frame index
            micPractice1v2.tStart = t  # local t and not account for scr refresh
            micPractice1v2.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(micPractice1v2, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.addData('micPractice1v2.started', t)
            # start recording with micPractice1v2
            micPractice1v2.start()
            micPractice1v2.status = STARTED
        if micPractice1v2.status == STARTED:
            # update recorded clip for micPractice1v2
            micPractice1v2.poll()
        if micPractice1v2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > micPractice1v2.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
                micPractice1v2.tStop = t  # not accounting for scr refresh
                micPractice1v2.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.addData('micPractice1v2.stopped', t)
                # stop recording with micPractice1v2
                micPractice1v2.stop()
                micPractice1v2.status = FINISHED
    ...
    # tell mic to keep hold of current recording in micPractice1v2.clips and transcript (if applicable) in micPractice1v2.scripts
    # this will also update micPractice1v2.lastClip and micPractice1v2.lastScript
    micPractice1v2.stop()
    tag = data.utils.getDateStr()
    micPractice1v2Clip = micPractice1v2.bank(
        tag=tag, transcribe='None',
        config=None
    )
    trialsPractice.addData('micPractice1v2.clip', os.path.join(micPractice1v2RecFolder, 'recording_micPractice1v2_%s.wav' % tag))
    # Run 'End Routine' code from codeSavePractice
    clipFilename = f"recording_Practice1v2_p{participant_number}_tr{str(count)}_{respModal}_{correctAns}_%s.wav" % tag
    #"recording_" + str(count) + ".wav"
    micPractice1v2.lastClip.save(os.path.join(micPractice1v2RecFolder, clipFilename))
...
routineTimer.reset()
# save micPractice1v2 recordings
for tag in micPractice1v2.clips:
    for i, clip in enumerate(micPractice1v2.clips[tag]):
        clipFilename = 'recording_micPractice1v2_%s.wav' % tag
        # if there's more than 1 clip with this tag, append a counter for all beyond the first
        if i > 0:
            clipFilename += '_%s' % i
        clip.save(os.path.join(micPractice1v2RecFolder, clipFilename))
