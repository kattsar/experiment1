...
```{r}
pilot1 <- read.csv("D:\\GitHub\\experiment1\\Analysis\\pilot000101(9028)\\000101_REPSWITCH1_version1_list1_2023-09-28_10h57.15.798.csv", header = TRUE, sep = ";", encoding = "UTF-8") %>% 
  rename(RespModal = respModal,
         ...
         Target = target) %>% 
  select(-c(1, 16:43, 54, 57:70, 72, 75, 77)) %>% 
  filter(!row_number() %in% c(1, 2, 411)) %>% 
...
#if RespModal speak and KeyResp_keys != "None", "0", "1" for Modality_Correct
#move RespModal after SubjResponse
#if backspace in KeyResp_keys, Error_corrected 1
#can R calculate duration for type trials automatically?
#check again item correct for speak trials

write.csv(pilot1, "D:\\GitHub\\experiment1\\Analysis\\pilot000101(9028)\\pilot000101(9028)_cleaned.csv", fileEncoding = "UTF-8")
```


```{r}
coded_pilot1 <- readxl::read_xlsx("D:\\GitHub\\experiment1\\Analysis\\pilot000101(9028)\\pilot000101_coded.xlsx", sheet = 1)
```

```{r}
merged_pilot1 <- merge(x = pilot1, y = coded_pilot1, by = "Trial_no", all = TRUE) %>% 
  select(-c(24:31, 34:41, 43:51, 53, 55)) %>% 
...
write_csv(merged_pilot1, "D:\\GitHub\\experiment1\\Analysis\\pilot000101(9028)\\pilot000101_merged_coded.csv")
```
